...
loc_A, loc_B, loc_C, loc_D, loc_E  = (0, 1), (1, 1), (2, 1), (1, 0), (1, 2)  # The five locations for the Vacuum world

def FiveRoomRandomAgent(actions):
    """An agent that chooses an action at random, ignoring all percepts.
    """
    return lambda percept: random.choice(actions)
...
def FiveRoomRandomAgent():
    ...
    >>> environment.add_thing(agent)
    >>> environment.run()
    >>> environment.status == {(0, 1) : 'Clean' , (1, 1) : 'Clean', (2, 1) : 'Clean', (1, 0) : 'Clean', (1, 2) : 'Clean'}
    True
    """
...
def FiveRoomReflexAgent():
    ...
    def program(percept):
        ...
        if status == 'Dirty':
            ...
            return location.random(loc_A, loc_C, loc_D, loc_E)
...
class FiveRoomVacuumEnvironment(Environment):
    """This environment has five locations, A,B,C,D and E. Each can be Dirty
    or Clean. The agent perceives its location and the location's
    status. This serves as an example of how to implement a simple
    Environment."""
    ...
    def __init__(self):
        ...
        self.status = {loc_A: 'Dirty', loc_B: 'Dirty', loc_C: 'Dirty', loc_D: 'Dirty', loc_E: 'Dirty'}

    def thing_classes(self):
        return [Wall, Dirt, ReflexVacuumAgent, RandomVacuumAgent, ModelBasedVacuumAgent]

    def percept(self, agent):
        status = ('Dirty' if self.some_things_at[agent.location] else 'Clean')
        return status
